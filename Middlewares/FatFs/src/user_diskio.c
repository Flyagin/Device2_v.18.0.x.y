...
)
{
  ...
  while (count_tmp != 0)
  {
    fs_temporary_address = address_df; //адреса з якої треба зчитувати
    fs_count_to_transfer = ((count_tmp >= SIZE_PAGE_DATAFLASH_2) ? SIZE_PAGE_DATAFLASH_2 : count_tmp); //кількість байт
    
    //подаємо команду читати
#ifndef I2C_EEPROM
    mutex_spi1 = true;
#endif
    ...
    //декрементуємо кіллькіть байт для читання
...
{ 
  ...
  unsigned int fs_count_to_transfer_tmp; //Потібні для забезпечення сумісночсті з модулями DataFlash, які написані давніше
  while (count_tmp != 0)
  {
    fs_temporary_address = address_df; //адреса з якої треба записувати
