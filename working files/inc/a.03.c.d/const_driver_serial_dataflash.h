...
30(с)х50(періодів)х16(виборок на період)х(8(канали) + 17(двобайтні слова дискретних сигналів))
х2(байти на число) = 1200000(байти)
...
аналоговий запис становить 1200060/512 = 2343,8671875 сторінки. Значить буде вестися 
запис у 2344 сторінки DF.
...
Тоді для запису 2344 сторінок треба буде
2344(сторінки)х36(мс) = 84 384(мс)=84,384(с)

Тобто процес запису максимального аналогового запису буде тривати 84,384(с)

Швидкість "заповнення" миттєвими виборками і дискретними сигналами з вимірювальної системи:
(8(канали) + 17(двобайтні слова дискретних сигналів))х2(байти на значення)/1,25(мс
- інтервал між виборками при частоті 50Гц і 16 виборок на період) = 40 (байт/мс)
...
2344(сторінки)х512(байт на сторінку)/84384(мс) = 14,(2) (байт/мс)
...
буде виконуватися зі швидкістю: 40 - 14,(2) = 25,78(байт/мс)
...
25000(мс)*25.77(байт/мс) = 644500(байт) <= 644500(байт)
644500(байт)/512(байт/стор.) = 1258,7890625(сторінок) < 1259(сторінок)
...
(1259 + 1) сторінки DF). Ще одну сторінку до 1260 теоретично розрахованих я додав 
для перестаховки.

Тобто розмір має бути
5(с - макс. доавар. мас.)х50(пром.частота)х16(кільк. вибор.за пер. для ан.р.)х(8(канали) + 17(двобайтні слова дискретних сигналів)) +1261*[512(байт)/2(байт на одне миттєве значення)](кількість миттєвих значень на сторінці)=
= 100000 +1261х256 = 422816
...
1261(сторінок)х256(слів) = 322816 (слів)

322816/(8 + 17)= 12912,64
Якщо ділення не є ціле число, то треба провести корекцію, щоб Х/(8 + 17)
давало ціле число, причому Х > 322816 - тобто беремо найслижче число X = 322825

таким чином
100000 + 322825 = 422825

!!!Вищевикладена дає мені підставу ширину масиву встановити 422825 елементи (двохбайтені,
так як кожна виборка складається із двох байт)!!!

*/

#define SIZE_BUFFER_FOR_AR    258725/*422825*/
